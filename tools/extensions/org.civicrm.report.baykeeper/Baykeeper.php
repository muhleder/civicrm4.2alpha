    ...
    function alterDisplay( &$rows ) {
        ...
        foreach ( $rows as $rowNum => $row ) {
            ...
            if ( array_key_exists( 'first_contribution', $row ) ||
                ...
                if ( $details['first'] ) {
                    $rows[$rowNum]['first_contribution'  ] =
                        CRM_Utils_Money::format( $details['first']['total_amount'] ) .
                        ' - ' .
                        CRM_Utils_Date::customFormat($details['first']['receive_date'], $config->dateformatFull );
                }
                if ( $details['last'] ) {
                    $rows[$rowNum]['last_contribution'] =
                        CRM_Utils_Money::format( $details['last']['total_amount'] ) .
                        ' - ' .
                        CRM_Utils_Date::customFormat($details['last']['receive_date'], $config->dateformatFull );
                }
            }
            ...
            if ( array_key_exists('soft_credit_display_name', $row) && 
                ...
                $rows[$rowNum]['soft_credit_display_name_creditor_hover'] =  
                    ts("View Contact Summary for this Soft Credit.");
            }
			...
			if ( array_key_exists('civicrm_contact_hon_mem_display_name', $row) &&
                ...
                $rows[$rowNum]['civicrm_contact_hon_mem_display_name_hover'] =  
                    ts("View Contact Summary for this Honor/Memory Contribution.");
            }
            ...
            if ( $value = CRM_Utils_Array::value( 'civicrm_contribution_honor_type_id', $row ) &&
				...
				} else if ($rows[$rowNum]['civicrm_contribution_honor_type_id'] == 2) {
            ...
            if ( ( $value = CRM_Utils_Array::value( 'civicrm_contribution_total_amount', $row ) ) && 
                ...
                $rows[$rowNum]['civicrm_contribution_total_amount_hover'] =  
